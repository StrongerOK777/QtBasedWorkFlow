        ...
        if (const QStyleOptionMenuItem* mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option))
        {
            ...
            else
            {
                ...
                if (mopt->menuHasCheckableItems)
                {
                    ...
                    painter->drawText(QRectF(menuRect.x() + contentPadding, menuRect.y(), _iconWidth, menuRect.height()), Qt::AlignCenter, mopt->checked ? ElaIconChar(ElaIconType::Check) : ElaIconChar(ElaIconType::None));
                    painter->restore();
                }
                else
                {
                ...
                if (mopt->menuItemType == QStyleOptionMenuItem::SubMenu)
                {
                    ...
                    painter->setFont(iconFont);
                    painter->drawText(QRect(menuRect.right() - 25, menuRect.y(), 25, menuRect.height()), Qt::AlignVCenter, ElaIconChar(ElaIconType::AngleRight));
                    painter->restore();
                }
                painter->restore();
            }
