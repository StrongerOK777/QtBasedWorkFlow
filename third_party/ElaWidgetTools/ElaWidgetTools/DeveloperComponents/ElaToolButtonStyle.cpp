...
void ElaToolButtonStyle::_drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    ...
    else if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu))
    {
        ...
        int indicatorWidth = painter->fontMetrics().horizontalAdvance(ElaIconChar(ElaIconType::AngleDown));
        ...
        painter->drawText(expandIconRect, Qt::AlignCenter, ElaIconChar(ElaIconType::AngleDown));
        painter->restore();
    }
}
