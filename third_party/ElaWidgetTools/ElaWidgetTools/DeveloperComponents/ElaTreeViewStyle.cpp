    ...
    case QStyle::PE_IndicatorBranch:
    {
        ...
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            if (vopt->state.testFlag(QStyle::State_Children))
            {
                ...
                painter->drawText(indicatorRect, Qt::AlignVCenter | Qt::AlignRight, vopt->state.testFlag(QStyle::State_Open) ? ElaIconChar(ElaIconType::AngleDown) : ElaIconChar(ElaIconType::AngleRight));
                painter->restore();
            }
        }
        return;
        ...
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            ...
            if (checkRect.isValid())
            {
                ...
                if (vopt->checkState == Qt::Checked)
                {
                    ...
                    painter->drawText(checkRect, Qt::AlignCenter, ElaIconChar(ElaIconType::Check));
                }
